...
var bootbox = __webpack_require__(6);
...
function getTicketInfo(sys_id, callback) {
    ...
    var theUrl = theProtocol + "//" + theHost + "/api/now/v2/table/incident?sysparm_limit=1&sysparm_count=true&sys_id=" + sys_id;
    //console.log("theUrl " + theUrl);
    fetch(theUrl, { credentials: 'same-origin' }).then(function (response) {
        if (response.status !== 200) {
            var errMsg = "ERROR: We asked for " + sys_id + " and got an HTML status code error " + response.status;
            throw errMsg;
        } else {
...
})(undefined, function init($, undefined) {
